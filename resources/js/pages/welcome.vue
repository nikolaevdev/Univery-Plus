<template>
   <div class="welcome-page">
      <div class="wrap d-flex flex-column">
        ...
        <router-link class="white-btn-reg mb-4" :to="{ name: 'statement.my' }" v-if="authenticated">{{ $t('statement_my_applications') }}</router-link>
        <router-link class="white-btn-reg mb-4" :to="{ name: 'register' }" v-if="!authenticated">{{ $t('welcome_register') }}</router-link>
        <ul class="list-group list-info-url">
          <router-link class="white-btn border " :to="{ name: 'info.news' }">{{ $t('welcome_info_url__news') }}</router-link>
          <router-link class="white-btn border " :to="{ name: 'info.articles.get', params: { id: 2 } }">{{ $t('welcome_info_url__regulation') }}</router-link>
          <router-link class="white-btn border " :to="{ name: 'info.documents' }">{{ $t('welcome_info_url__documents') }}</router-link>
        </ul>
        <footer class="mt-3">
        	<ul class="nav justify-content-center">
      	    <li class="nav-item" v-if="!authenticated">
              <router-link class="nav-link" :to="{ name: 'login' }">{{ $t('welcome_footer__login') }}</router-link>
            </li>
             <li class="nav-item">
               <router-link :to="{ path: '/help' }" class="nav-link">
                  {{ $t('help') }}
              </router-link>
            </li>
			    </ul>
