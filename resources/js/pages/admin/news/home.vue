    ...
    <div class="module-block bg-white rounded mb-4">
      ...
      <div class="bg-white p-3 border-bottom shadow-sm">
        <div class="d-flex flex-row-reverse">
          <select class="rounded-pill py-2 px-3 border tab-link" id="news_add_category" v-model="category_select">
      ...
      <template v-if="!isNull(news) && islength(news) && !error && !loading">
        ...
        <div class="d-flex flex-column p-3 rounded-lg mt-2">
          <div class="news-item p-3 border mb-3 d-flex" v-for="(item, index) in news" :key="item.id">
            <div class="d-flex flex-grow-1 flex-column title-wrap">
              <h5 class="title">
                <router-link :to="{ name: 'admin.news.get', params: { id: item.id } }">
                  {{item.name}}
                </router-link>
              </h5>
              <div class="d-flex flex-lg-row flex-sm-column">
                ...
                <div class="py-2 d-flex mr-2">
                  <span class="name font-weight-bold mr-1">{{ $t('news_author') }}</span>
                  <span class="font-italic">{{item.author_name}}</span>
                </div>
                <div class="py-2 d-flex mr-2">
                  <span class="name font-weight-bold mr-1">{{ $t('news_views') }}</span>
...
export default {
  ...
  metaInfo () {
    return { title: this.$t('admin_news') }
  },
