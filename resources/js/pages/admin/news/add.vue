					...
					<div>
						<div class="row">
							...
							<div class="col-12 col-lg-9">
				            	...
				            	<input id="news_title" class="form-control mr-4 rounded-pill" type="text" :placeholder="$t('admin_news_title')" v-model="name">
				            </div>
						    </div>
				        <div id="editor-js" class="p-3">
							...
							<div class="col-12 col-lg-3">
								<div class="wrap-100">
									<form class="p-3">
										<div class="form-group">
										  <button type="button" class="nav-link btn btn-primary w-100 rounded-pill" @click="add_news()">{{ $t('admin_save') }}</button>
									  </div>
					    		  <div class="form-group">
									    <label for="news_add_state">{{ $t('admin_news_state') }}</label>
									    <select class="form-control rounded-pill" id="news_add_state" v-model="state">
									    ...
									    <select class="form-control rounded-pill" id="news_add_category" v-model="category_select">
...
export default {
    ...
    metaInfo () {
        return { title: this.$t('admin_news_add') }
    },
    ...
    data() {
	    return {
	    	...
	    	error: null,
	    	editor: null,
	    	name: localdata.get('admin.new.news.name')[1],
	    	datanews:  localdata.get('admin.new.news.text')[1],
	    	state: localdata.get('admin.new.news.state')[1],
	    	category_select: localdata.get('admin.new.news.category_select')[1],
	    }
	},
    ...
    mounted(){
    	...
    	this.editor = new EditorJS({
		    ...
		    onChange: (data) => {
				...
				.then((savedData) => {
				    localdata.put('admin.new.news.name', this.name);
				    localdata.put('admin.new.news.state', this.state);
				    localdata.put('admin.new.news.category_select', this.category_select);
				    localdata.put('admin.new.news.text', savedData);
				}).catch((error) => {
					this.$notify({
		              group: 'news',
		              type: 'error',
        ...
        add_news() {
        	...
        	this.editor.saver.save()
                ...
                axios.post('/api/admin/news.add', {
                    ...
                    if (response.data.ok) {
                    	localdata.delete('admin.new.news.text');
					            localdata.delete('admin.new.news.name');

					            localdata.delete('admin.new.news.state');
					            localdata.delete('admin.new.news.category_select');

                    	this.$router.push({ name: `admin.news.get`, params: { id: response.data.new_id }})
                    }
				        })
				        .catch(error => {
