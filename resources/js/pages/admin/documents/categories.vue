<template>
  <div class="module">
  	<div class="mb-3 d-flex justify-content-between bg-secondary p-3 flex-row-reverse">
  		<v-button :nativeType="'button'" v-on:click.native="add_()" class="py-2 px-3 mr-2 rounded-pill font-weight-bold">{{ $t('admin_add') }}</v-button>
  	</div>
          ...
          <div class="p-3 d-flex d-flex align-items-center" v-if="show_add_Field()">
            <form class="list-group-item-edit form-inline flex-grow-1">
              <div class="form-group mr-2">
                <input type="text" class="form-control form-control" v-model="addFieldValue">
              </div>
            </form>
            <div class="btn-group rounded" role="group" aria-label="">
              <v-button type="primary" nativeType="button" v-on:click.native="save_add_Field()">{{$t('admin_add')}}</v-button>
              <v-button type="primary" nativeType="button" v-on:click.native="add_close()">{{$t('admin_close')}}</v-button>
            </div>
          </div>
...
export default {
  ...
  metaInfo () {
    return { title: this.$t('admin_documents_categories') }
  },
  ...
  methods: {
    ...
    save_edit_Field(index, record) {
      ...
      axios.post('/api/admin/documents.categories.save', {
        id: record,
    ...
    save_add_Field() {
      ...
      axios.post('/api/admin/documents.categories.add', {
      ...
      if(!id || index === null)
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            axios.post('/api/admin/documents.categories.delete', {
              id: id,
              headers: {
                  'Content-Type': 'application/json'
              }
